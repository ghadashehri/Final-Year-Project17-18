...
In this class we use the Bi-grams vectors, to represent our feature set.
It starts by splitting up feature set to vectors and labels, then we estimate
value of Eps, using plot produced byKDistance_BiGram class. We run the algorithm
...
#gGet features from Feature Extraction classes
data = getBiGramVec()
encod = LabelEncoder()
...
# compute DBSCAN
dbscn = DBSCAN(eps=19.95, min_samples=minPts).fit(X)
labels = dbscn.labels_

std_dbscn = DBSCAN(eps=6.74, min_samples=minPts).fit(std_X)
