...
@author: Ghadah
This class was produced for experiment 3.
it performs DBSCAN on frequency vectors, but uses
Manhattan distance as metric function.
It plots 3 graphs, each representing a validity measure

NOTE: The values are already assigned
'''
...
for i in range(0, 60):
    eps.append(epsilon)
    model = DBSCAN(eps=epsilon, min_samples=8, metric='manhattan').fit(X)
    print(' ', i, ' ', len(set(model.labels_)) -
          (1 if -1 in model.labels_ else 0))
    print(' ', metrics.homogeneity_completeness_v_measure(Y, model.labels_))
    print(' ', metrics.precision_recall_fscore_support(Y, model.labels_,
                                                       average='micro'))
